...
import io.avand.repository.JobEntityRepository;
 ...
 *
 * @see JobEntityResource
 */
@RunWith(SpringRunner.class)
@SpringBootTest(classes = VoldemortApp.class)
public class JobEntityResourceIntTest {
    ...
    public void setup() {
        ...
        final JobEntityResource jobEntityResource = new JobEntityResource(jobEntityRepository);
        this.restJobEntityMockMvc = MockMvcBuilders.standaloneSetup(jobEntityResource)
    ...
    public static JobEntity createEntity(EntityManager em) {
        JobEntity jobEntity = new JobEntity()
            .name(DEFAULT_NAME);
        // Add required entity
        CandidateEntity candidate = CandidateEntityResourceIntTest.createEntity(em);
        em.persist(candidate);
        em.flush();
        jobEntity.setCandidate(candidate);
        // Add required entity
        CompanyEntity company = CompanyEntityResourceIntTest.createEntity(em);
        em.persist(company);
        em.flush();
    ...
    public void createJobEntity() throws Exception {
        int databaseSizeBeforeCreate = jobEntityRepository.findAll().size();
        ...
        List<JobEntity> jobEntityList = jobEntityRepository.findAll();
    ...
    public void updateJobEntity() throws Exception {
        // Initialize the database
        jobEntityRepository.saveAndFlush(jobEntity);
        int databaseSizeBeforeUpdate = jobEntityRepository.findAll().size();

        // Update the jobEntity
        JobEntity updatedJobEntity = jobEntityRepository.findOne(jobEntity.getId());
    ...
    public void deleteJobEntity() throws Exception {
        // Initialize the database
        jobEntityRepository.saveAndFlush(jobEntity);
        int databaseSizeBeforeDelete = jobEntityRepository.findAll().size();
